...
def list_apps_service_plan(resource_group_name, name):
    client = web_client_factory()
    apps = client.app_service_plans.list_web_apps(resource_group_name, name)
    return apps

def restart_apps_service_plan(resource_group_name, name):
    client = web_client_factory()
    apps = client.app_service_plans.list_web_apps(resource_group_name, name)
    for app in apps:
        parse = _extract_resource_group_name(app.id)
        client.web_apps.restart(parse[0], parse[1])
    apps = client.app_service_plans.list_web_apps(resource_group_name, name)
    return apps

def start_apps_service_plan(resource_group_name, name):
    client = web_client_factory()
    apps = client.app_service_plans.list_web_apps(resource_group_name, name)
    for app in apps:
        parse = _extract_resource_group_name(app.id)
        client.web_apps.start(parse[0], parse[1])
    apps = client.app_service_plans.list_web_apps(resource_group_name, name)
    return apps

def stop_apps_service_plan(resource_group_name, name):
    client = web_client_factory()
    apps = client.app_service_plans.list_web_apps(resource_group_name, name)
    for app in apps:
        parse = _extract_resource_group_name(app.id)
        client.web_apps.stop(parse[0], parse[1])
    apps = client.app_service_plans.list_web_apps(resource_group_name, name)
    return apps

def _extract_resource_group_name(id):
    split = id.lower().split('/')
    return split[split.index('resourcegroups') + 1], split[split.index('resourcegroups') + 5]
